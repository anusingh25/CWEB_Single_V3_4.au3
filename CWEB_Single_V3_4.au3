...
Global $value = 20
Global $name=""
Local $mGUI = GUICreate("Move", 40, 60, 1830, 948, $WS_SIZEBOX)

Local $idInput = GUICtrlCreateInput($value, 20, 40, 10, 10)
Local $idCheckbox = GUICtrlCreateCheckbox("AX", 70, 5, 30, 25)
...
Func ShowMessage()
	Global $line = StringStripWS(StringReplace(StringReplace(InputBox("CWEB", "Enter Part Number", StringStripWS(StringReplace(StringReplace(StringReplace(StringStripWS(ClipGet(), $STR_STRIPLEADING + $STR_STRIPTRAILING ), "-", " "), " ", " "), ".", ""), $STR_STRIPTRAILING), "", 150, 130, 1990, 948), "-", "_"), " ", "_"), $STR_STRIPALL)
	ControlSetText("Locate","","[CLASS:Edit; INSTANCE:6]","")
	ControlSend("Locate","","[CLASS:Edit; INSTANCE:6]",$line&"{ENTER}")
	;sleep(100)
	;ControlClick("Locate","","[CLASS:Button; INSTANCE:14]","left")
	sleep(400)
	ControlClick("Locate","","[CLASS:SysListView32; INSTANCE:1]","left",1,60,30)
	if $ax=1 then
	Controlsend("‪Microsoft Dynamics AX‬ - ‎‪UniCarriers Americas Corporation‬","","WindowsForms10.EDIT.app.0.34f5582_r35_ad11",StringReplace($line,"_","-"))
	Controlsend("‪Microsoft Dynamics AX‬ - ‎‪UniCarriers Americas Corporation‬","","WindowsForms10.EDIT.app.0.34f5582_r35_ad11","{ENTER}")
	EndIf
	;WinActivate("Locate")
	ClipPut($line)
EndFunc   ;==>ShowMessage
...
Func query()
	ControlSetText("NX I-deas 6.2", "", "[CLASS:Edit; INSTANCE:4]", "/li in")
	ControlSend("NX I-deas 6.2", "", "[CLASS:Edit; INSTANCE:4]", "{ENTER}")
	sleep(1000)
...
Local $hWnd = WinActivate("NX I-deas 6.2 :    Nissan Forklift -Install #30846-Hotline 1-800-955-0000 : users.singha")
   ...
   Local $iResult = 0
        ...
        $iResult = MsgBox(BitOR($MB_SYSTEMMODAL, $MB_OKCANCEL), "",$aResult[$n-1])
        If $iResult == $IDOK Then
			;ShellExecute("J:\Engineers\Cpengr\Anand\"&$sFileName)
	Global $line = StringStripWS(StringReplace($aResult[$n-1],"-"," "), $STR_STRIPALL)

	;MsgBox(4096,"hi",$line)
	ControlSetText("Locate","","[CLASS:Edit; INSTANCE:6]",$line)
	ControlClick("Locate","","[CLASS:Button; INSTANCE:14]","left")
	sleep(400)
	ControlClick("Locate","","[CLASS:SysListView32; INSTANCE:1]","left",1,60,35)
	;WinActivate("Locate")
	Controlsend("‪Microsoft Dynamics AX‬ - ‎‪UniCarriers Americas Corporation‬","","WindowsForms10.EDIT.app.0.34f5582_r35_ad11",StringReplace($line,"_","-"))
	Controlsend("‪Microsoft Dynamics AX‬ - ‎‪UniCarriers Americas Corporation‬","","WindowsForms10.EDIT.app.0.34f5582_r35_ad11","{ENTER}")
	ClipPut($line)
